...
function CategoryIcon({
  ...
  const imgCls   = size === "lg" ? "w-8 h-8 object-contain" : "w-5 h-5 object-contain";
  const svgCls   = size === "lg" ? "w-8 h-8" : "w-5 h-5";
  const emojiCls = size === "lg" ? "text-3xl leading-none" : "text-lg leading-none";
...
/** Badge grid kecil: row/col. Merah jika null. */
function GridBadge({ row, col }: { row: number | null; col: number | null }) {
  const missing = row == null || col == null;
  return (
    <span
      className={[
        "inline-flex items-center gap-0.5 font-mono text-[9px] font-semibold px-1 py-0.5 rounded",
        missing
...
export default function AdminSearchModal({ onEdit }: AdminSearchModalProps) {
  ...
  // Fetch fasilitas dengan debounce
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setIsLoading(true);
      try {
        const params = new URLSearchParams({ terminal: activeTerminal });
  ...
  const handleKeyPress  = (key: string)  => setQuery((p) => p + key);
  const handleBackspace = ()              => setQuery((p) => p.slice(0, -1));
  const handleSpace     = ()              => setQuery((p) => p + " ");
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Cari fasilitas (Admin)"
    >
      {/* Backdrop */}
      ...
      <div className="relative z-10 flex items-start gap-3">
        ...
        >
          ...
          >
            <div className="overflow-y-auto h-56 p-2 pr-1 [&::-webkit-scrollbar]:hidden">

              {/* Loading skeleton */}
              {isLoading && (
                <div className="grid grid-cols-3 gap-1.5">
                  {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} className="h-14 rounded-lg bg-white/70 animate-pulse" />
                  ))}
                </div>
              )}

              {/* Hasil — 3 kolom, tiap kartu tampilkan grid debug */}
              {!isLoading && results.length > 0 && (
                <div className="grid grid-cols-3 gap-1.5">
                  {results.map((facility) => {
                    const row = facility.gridRow != null ? Math.round(facility.gridRow) : null;
                    const col = facility.gridCol != null ? Math.round(facility.gridCol) : null;
                    const noGrid = row == null || col == null;
                    ...
                    return (
                      <button
                        ...
                        className={[
                          ...
                          "h-14 px-2.5 py-1.5 rounded-lg gap-0.5",
                          noGrid
                            ? "bg-amber-50 hover:bg-amber-100 active:bg-amber-200 border border-amber-200"
                      ...
                      >
                        {/* Baris atas: icon kecil + nama */}
                        <div className="flex items-center gap-1.5 w-full min-w-0">
                          <span
                            className="shrink-0 w-5 h-5 rounded flex items-center justify-center"
                            style={{ backgroundColor: facility.category?.color ?? "#38bdf8" }}
                          >
                            <CategoryIcon icon={facility.category?.icon ?? null} name={facility.category?.name ?? ""} size="md" />
                          </span>
                          <span className="text-gray-800 text-[10px] font-semibold leading-tight line-clamp-1 flex-1 min-w-0 tracking-tight">
                            {facility.name}
                          </span>
                        </div>

                        {/* Baris bawah: grid badge + kode */}
                        <div className="flex items-center justify-between w-full gap-1">
                          <GridBadge row={row} col={col} />
                          <span className="text-gray-400 text-[8px] font-mono truncate max-w-[40%]">
                            {facility.code}
                          </span>
                        </div>
                      </button>
          ...
          <div className="flex justify-center px-4 mb-3">
            ...
            >
              <svg className="shrink-0 w-4 h-4 text-gray-400" xmlns="http://www.w3.org/2000/svg"
              ...
              {query.length > 0 && (
                <button onClick={handleClearQuery} aria-label="Hapus pencarian"
                  ...
                  <svg className="w-3.5 h-3.5 text-white" xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" aria-hidden="true">
          ...
          {/* ── Virtual keyboard — QWERTY + numbers + backspace ── */}
